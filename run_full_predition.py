...
def predict_padn_threshold_probs(padn_df, flat_ct_dir, config):
    ...
    model_main = load_module_from_file("phase_prior_guided_change_main", MODEL_FILE)
    ...
    model = model_main.build_model_from_config(
        {
    ...
    state_dict = model_main.extract_model_state(checkpoint)
    incompatible = model.load_state_dict(state_dict, strict=True)
    if incompatible.missing_keys or incompatible.unexpected_keys:
        raise RuntimeError(
            "Checkpoint is incompatible with phase_prior_guided_change: "
    ...
    with torch.no_grad():
        for pre_ct, post_ct, _labels, volumes, pre_masks, post_masks in loader:
            ...
            probs, _preds = model_main.decode_ordinal_predictions(outputs)
...
if __name__ == "__main__":
    CONFIG = {
        ...
        "padn_checkpoint_path": str(BASE_DIR / "results" / "修改架构后" / "best_kappa.pth"),
        "padn_target_shape": (182, 218, 182),
        "padn_batch_size": 1,
        "padn_num_workers": 0,
        "padn_dropout": 0.6,
        "padn_use_region_attention": True,
        "padn_model_mode": "phase_prior_guided_change",
        "padn_disable_all_priors": False,
